module Utility exposing
  ...
  , minimality, mkConn, mkAutomatonGraph, dummy_uuid, ag_cmp_and_equals
  , uuid_from_int
  )
...
ag = mkAutomatonGraph << Data.mkAG_input

dfa : String -> List Int -> Data.DFARecord {}
    ...
    getEdges g =
      ...
      |> List.map
        (\{ from, to, label } ->
          ...
          , to
          , AutoSet.toList label
            |> List.map Data.transitionToString
            |> List.sort
          )
        )
      |> List.sortBy (\(_, _, v) -> v)
